...
namespace TCU_WFA
{
    public partial class FormAlertas : DefaultForm
    {
        ...
        private const string QUERY_SELECT_VACAS_ALERTAS_DESTETES = "SELECT v.PK_NUMERO_TRAZABLE as 'Id', v.NOMBRE AS 'Nombre', v.FECHA_NACIMIENTO AS 'Fecha nacimiento', r.RAZA AS 'Raza', v.PESO AS 'Peso', d.ESTADO AS 'Estado desarrollo', v.CARACTERISTICAS AS 'Caracteristicas', mP.MODO_PRENNES AS 'Modo preñez', v.FK_NUMERO_TRAZABLE_VACA AS 'Id madre', v.FK_NUMERO_TRAZABLE_TORO AS 'Id padre', v.ACTIVA AS '¿Activa?' FROM dbo.[VACA] v LEFT JOIN dbo.[MODO_PRENNES] mP ON v.FK_ID_MODO_PRENNES = mP.PK_ID_MODO_PRENNES LEFT JOIN dbo.[RAZA] r ON v.FK_ID_RAZA = r.PK_ID_RAZA LEFT JOIN dbo.DESARROLLO d ON v.FK_ID_DESARROLLO = d.PK_ID_DESARROLLO WHERE v.ACTIVA = 1 AND mP.MODO_PRENNES = 'No preñada' AND DATEADD(DAY, @diasParaDestete, (SELECT MAX(p.PK_FECHA) FROM dbo.[PARTO] p WHERE p.PK_FK_NUMERO_TRAZABLE_VACA = v.PK_NUMERO_TRAZABLE AND p.MUERTE_PREMATURA = 0)) <= GETDATE() AND NOT EXISTS(SELECT fD.PK_FK_NUMERO_TRAZABLE_VACA FROM dbo.[FECHAS_DESTETES] fD WHERE v.PK_NUMERO_TRAZABLE = fD.PK_FK_NUMERO_TRAZABLE_VACA AND fD.PK_FECHA > (SELECT MAX(p.PK_FECHA) FROM dbo.[PARTO] p WHERE p.PK_FK_NUMERO_TRAZABLE_VACA = v.PK_NUMERO_TRAZABLE AND p.MUERTE_PREMATURA = 0));";
        ...
        private const string DIAS_PARA_DESTETE_PARAM = "@diasParaDestete";
        ...
        private const string LABEL_ALERTAS_DESTETE_BASE = "Alerta, las siguientes vacas ya alcanzaron los ... días luego de su último parto";
        ...
        private void ActualizarLabels()
        {
            ...
            labelAlertaDestetes.Text = LABEL_ALERTAS_DESTETE_BASE.Replace(LABEL_ALERTAS_PARAM, config.ObtenerConfig(ProgramConfiguration.LLAVE_ALERTA_DESTETE));
        }
        ...
        private void LlenarDataGridViews()
        {
            ...
            Utilities.LlenarDataGridView(QUERY_SELECT_VACAS_ALERTAS_DESTETES.Replace(DIAS_PARA_DESTETE_PARAM, config.ObtenerConfig(ProgramConfiguration.LLAVE_ALERTA_DESTETE)), dataGridViewAlertasDestetes);
        }
    }
}
